...
class TCD_FAQ_Accordion_Widget extends \Elementor\Widget_Base {
    ...
    protected function register_controls() {
        ...
        $categories  = get_terms( array( 'taxonomy' => 'faq-category', 'hide_empty' => false ) );
        ...
        $this->add_control( 'icon_color', array(
            'label'     => 'Color',
            'type'      => \Elementor\Controls_Manager::COLOR,
            'selectors' => array( '{{WRAPPER}} .tcd-faq-icon svg' => 'stroke: {{VALUE}};' ),
        ) );

        $this->add_control( 'icon_color_active', array(
            'label'     => 'Active Color',
            'type'      => \Elementor\Controls_Manager::COLOR,
            'selectors' => array( '{{WRAPPER}} .tcd-faq-item.is-open .tcd-faq-icon svg' => 'stroke: {{VALUE}};' ),
        ) );
    ...
    protected function render() {
        ...
        $icon_svg = '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><line x1="12" y1="5" x2="12" y2="19" stroke="currentColor" stroke-linecap="round"/><line x1="5" y1="12" x2="19" y2="12" stroke="currentColor" stroke-linecap="round"/></svg>';
        ...
        if ( $group_by_cat ) {
            ...
            foreach ( $faqs as $faq ) {
                $terms = get_the_terms( $faq->ID, 'faq-category' );
            ...
            echo '<div class="tcd-faq-accordion" data-collapse="' . esc_attr( $collapse ) . '">';
            $faq_index = 0;
            ...
            foreach ( $categorized as $cat_data ) {
                ...
                foreach ( $cat_data['faqs'] as $faq ) {
                    $this->render_faq_item( $faq, $icon_svg, $first_open && $faq_index === 0 );
                    $schema_items[] = $faq;
                    $faq_index++;
                }
                echo '</div>';
            ...
            foreach ( $faqs as $index => $faq ) {
                $this->render_faq_item( $faq, $icon_svg, $first_open && $index === 0 );
                $schema_items[] = $faq;
            }
            echo '</div>';
        }
    ...
    private function render_faq_item( $faq, $icon_svg, $is_open = false ) {
        ...
        $max_height  = $is_open ? 'max-height:2000px;' : 'max-height:0;';
        ...
        echo '<span class="tcd-faq-icon">' . $icon_svg . '</span>';
        echo '</button>';
